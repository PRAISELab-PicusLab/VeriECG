...
class CLIPECGTower(nn.Module):
    ...
    def load_model(self, model_name):
        ...
        # carica su CPU, poi sposta su CUDA se serve
    ...
    def forward(self, ecgs):
        ...
        if type(ecgs) is list:
            ecg_features = []
            for ecg in ecgs:
                ecg_feature = self.ecg_tower(
                    ecg.to(device=self.device, dtype=self.dtype).unsqueeze(0),
